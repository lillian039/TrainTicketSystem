...
private:
    ...
    int rear_tree, rear_leaf, sum_data;//B+树主体与叶子文件尾所在位置+B+树中总数据条数
    const int len_of_head_leaf = 2 * sizeof(int);//叶子节点所在文件头的长度，包含总数据条数与文件尾所在位置
    const int len_of_head_tree = 2 * sizeof(int);//B+树节点所在文件头的长度，包含根节点所在位置与文件尾所在位置
    sjtu::vector<int> empty_tree;//存放回收的树节点空间位置
    ...
    struct Leaf {
        ...
        int n;// n节点中value的个数
        std::pair<Key, T> value[L];//存放的数据 0 base
    };
