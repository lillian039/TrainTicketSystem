...
public:
    ...
    string add_train(
        ...
        if (x.first) return "好像有这辆车捏"; // trainid 存在
    ...
    string delete_train(const string &trainid) {
        sjtu::my_str<20> ti = trainid;
        int th = ti.toint();
        std::pair<bool, std::pair<int, int>> x = bpt_train_information.find(th);
        if (!x.first) return "好像没有这辆车捏"; // trainid 不存在
        train u = train_information.find(x.second.first);
        if (u.release) return "发布过啦，删不了了ovo"; //车次已发布
    ...
    string release_train(const string &trainid) {
        sjtu::my_str<20> ti = trainid;
        int th = ti.toint();
        std::pair<bool, std::pair<int, int>> x = bpt_train_information.find(th);
        if (!x.first) return "好像没有这辆车捏"; // trainid 不存在
        train u = train_information.find(x.second.first);
        if (u.release) return "你是不是忘了这辆车发布过啦ovo"; //车次已发布
        u.release = 1;
    ...
    string query_train(const string &trainid, const date &depart) {
        ...
        for (int i = 0; i < u.station_num; i++) {
            ...
            if (i == u.station_num - 1) ans += "x<br>";
            else ans += std::to_string(v.a[i]) + "<br>";
        }
            ...
            const date &depart, const bool &compare) // compare=0按照time排序，compare=1按照cost排序
    {
        ...
        string ans = std::to_string(z) + "<br>";
        for (int i = 0, u; i < z; i++) {
            u = id[i];
            ans += trainid[u] + " " + start_station + " " + time[u].first.tostr() + " -> " +
                   end_station + " " + time[u].second.tostr() + " " +
                   std::to_string(price[u]) + " " + std::to_string(seat_num[u]) + "<br>";
        }
        return ans;
    ...
    string query_transfer(
        ...
        string ans = "";
        if (!fl) return "没有车次来资瓷这两站的换乘www<br>";
        else
            for (int u = 0; u < 2; u++)
