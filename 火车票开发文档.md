...
- `train.hpp`：车次类
  ...
  BPTtree<string,int> bpt_station_train;
  //每个车站对应哪些车次id位置
  
  BPTtree<string,std::pair<int,int>> bpt_train_information;
  //每个车次id对应数据库中位置 fisrt 对应火车信息，second对应车票信息
...
- `order.hpp`：订单类
  ...
  BPTtree<string,order> btp_user_order;
  //用用户id查询订单
  
  BPTtree<string,order> btp_train_order;
  //查询id车次的候补队列
